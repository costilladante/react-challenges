...
const LabelFilter = () => {
  ...
  const selectAnimalClass = (animalClass) => {
    setFilterTags((prevState) => {
      if (prevState.includes(animalClass)) {
        const result = prevState.filter((tag) => {
          return tag !== animalClass;
        });
        return result;
      }
      return [...prevState, animalClass];
    });
  };
  ...
  return (
    <Wrapper>
      <div data-testid='labels-wrapper-id' className='label-container'>
        {animalClasses.map((animalClass) => (
          <div
            ...
            className='label'
            style={{
              color: filterTags.includes(animalClass) ? '#fff' : '#333',
              backgroundColor: filterTags.includes(animalClass)
                ? '#333'
                : '#fff',
            }}
            key={animalClass}
            onClick={() => selectAnimalClass(animalClass)}
          >
            {animalClass}
          </div>
        ))}
      ...
      <div data-testid='tile-container-id' className='tile-container'>
        {animalData.map(
          (animal) =>
            (filterTags.length === 0 || filterTags.includes(animal.class)) && (
              <div
